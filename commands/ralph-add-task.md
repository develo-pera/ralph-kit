...
3. Read `.ralph/fix_plan.md`. If missing or `Status: BLOCKED`, stop and recommend `/ralph-define`.
...
- If the task is clearly too big for one Ralph loop (>1 sentence describing multiple actions), warn and suggest `/ralph-add-feature` instead, but still honor the user's choice if they proceed.
